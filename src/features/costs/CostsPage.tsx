...
import { calculateCostSummary, vatFromGross, type CostCategory, type ExpenseRecord, type SalaryRecord, type Supplier } from "./types";
import "./costs.css";
...
export function CostsPage({ property }: { property: Property }) {
  ...
  async function reload(){
    ...
    const [c,s,e,p]=await Promise.all([costsStore.listCategories(property.id),costsStore.listSuppliers(property.id),costsStore.listExpenses(property.id),costsStore.listSalaries(property.id)]);
  ...
  const comparisons=useMemo<YearCostComparison[]>(()=>{
    ...
    return comparisonYears.map(year=>{
      ...
      return {year,month:targetMonth,summary:calculateCostSummary(targetExpenses,targetSalaries),categoryTotals:new Map(categories.map(c=>[c.id,targetExpenses.filter(x=>x.categoryId===c.id).reduce((s,x)=>s+x.grossCents,0)]))};
    });
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">Controlo de custos</p><h1>Custos</h1><p>Despesas operacionais, salários, fornecedores e categorias, com comparação mensal entre anos.</p></div><div className="cost-filters"><label>Mês<input type="month" value={month} onChange={e=>setMonth(e.target.value)}/></label><button className="secondary-button" onClick={()=>setMonth("")}>Todos</button><label>Categoria<select value={categoryFilter} onChange={e=>setCategoryFilter(e.target.value)}><option value="all">Todas</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>{month&&<YearComparisonPicker options={availableYears} selected={comparisonYears} onChange={setComparisonYears}/>}</div></div>
...
function CategoriesTab({propertyId,categories,act}:{propertyId:string;categories:CostCategory[];act:(w:()=>Promise<void>)=>Promise<void>}){
 const [editing,setEditing]=useState<CostCategory|null>(null); const [name,setName]=useState("");const [description,setDescription]=useState("");
 function load(x?:CostCategory){setEditing(x??null);setName(x?.name??"");setDescription(x?.description??"");}
 return <div className="cost-two-column"><article className="panel"><h2>{editing?"Editar categoria":"Nova categoria"}</h2><div className="cost-form"><label>Nome<input value={name} onChange={e=>setName(e.target.value)}/></label><label>Descrição<textarea value={description} onChange={e=>setDescription(e.target.value)}/></label><button className="primary-button" disabled={!name.trim()} onClick={()=>void act(async()=>{await costsStore.saveCategory({id:editing?.id??crypto.randomUUID(),propertyId,name:name.trim(),description:description.trim(),active:true});load();})}>Guardar</button>{editing&&<button className="secondary-button" onClick={()=>load()}>Cancelar</button>}</div></article><article className="panel cost-table"><table><thead><tr><th>Categoria</th><th>Descrição</th><th></th></tr></thead><tbody>{categories.map(x=><tr key={x.id}><td><strong>{x.name}</strong></td><td>{x.description||"—"}</td><td><button onClick={()=>load(x)}>Editar</button><button onClick={()=>void act(()=>costsStore.deleteCategory(propertyId,x.id))}>Remover</button></td></tr>)}</tbody></table></article></div>
}

function SuppliersTab({propertyId,suppliers,act}:{propertyId:string;suppliers:Supplier[];act:(w:()=>Promise<void>)=>Promise<void>}){
 const empty:Supplier={id:"",propertyId,name:"",taxId:"",contactName:"",email:"",phone:"",address:"",website:"",paymentTermsDays:null,notes:"",active:true}; const [f,setF]=useState<Partial<Supplier>>(empty);
 const edit=(x?:Supplier)=>setF(x??empty); const field=(k:keyof Supplier)=>(e:any)=>setF(v=>({...v,[k]:e.target.value}));
 return <div className="cost-two-column"><article className="panel"><h2>{f.id?"Editar fornecedor":"Novo fornecedor"}</h2><div className="cost-form cost-form-grid"><label>Nome<input value={f.name??""} onChange={field("name")}/></label><label>NIF / VAT<input value={f.taxId??""} onChange={field("taxId")}/></label><label>Contacto<input value={f.contactName??""} onChange={field("contactName")}/></label><label>Email<input value={f.email??""} onChange={field("email")}/></label><label>Telefone<input value={f.phone??""} onChange={field("phone")}/></label><label>Website<input value={f.website??""} onChange={field("website")}/></label><label>Prazo pagamento (dias)<input type="number" value={f.paymentTermsDays??""} onChange={e=>setF(v=>({...v,paymentTermsDays:e.target.value?Number(e.target.value):null}))}/></label><label className="wide">Morada<input value={f.address??""} onChange={field("address")}/></label><label className="wide">Notas<textarea value={f.notes??""} onChange={field("notes")}/></label><button className="primary-button" disabled={!f.name?.trim()} onClick={()=>void act(async()=>{await costsStore.saveSupplier({...empty,...f,id:f.id||crypto.randomUUID(),propertyId,name:f.name!.trim()} as Supplier);edit();})}>Guardar</button>{f.id&&<button className="secondary-button" onClick={()=>edit()}>Cancelar</button>}</div></article><article className="panel cost-table"><table><thead><tr><th>Fornecedor</th><th>Contacto</th><th>NIF</th><th></th></tr></thead><tbody>{suppliers.map(x=><tr key={x.id}><td><strong>{x.name}</strong><br/><small>{x.email}</small></td><td>{x.contactName||x.phone||"—"}</td><td>{x.taxId||"—"}</td><td><button onClick={()=>edit(x)}>Editar</button><button onClick={()=>void act(()=>costsStore.deleteSupplier(propertyId,x.id))}>Remover</button></td></tr>)}</tbody></table></article></div>
}
...
function ExpensesTab({propertyId,categories,suppliers,rows,categoryName,supplierName,act}:{propertyId:string;categories:CostCategory[];suppliers:Supplier[];rows:ExpenseRecord[];categoryName:(id:string)=>string;supplierName:(id:string|null)=>string;act:(w:()=>Promise<void>)=>Promise<void>}){
 const [bulkMode,setBulkMode]=useState(false);const [edit,setEdit]=useState<ExpenseRecord|null>(null);const [date,setDate]=useState(new Date().toISOString().slice(0,10));const [desc,setDesc]=useState("");const [cat,setCat]=useState("");const [sup,setSup]=useState("");const [amount,setAmount]=useState("");const [vat,setVat]=useState("23");const [status,setStatus]=useState<"paid"|"pending">("paid");const [rec,setRec]=useState(false);const [invoice,setInvoice]=useState("");const [notes,setNotes]=useState("");
 ...
 return <><article className="panel"><div className="cost-entry-heading"><h2>{bulkMode?"Entrada em massa":edit?"Editar despesa":"Nova despesa"}</h2><label className="cost-mass-toggle"><input type="checkbox" checked={bulkMode} onChange={e=>toggleBulk(e.target.checked)}/> Entrada em massa</label></div>{bulkMode?<BulkExpenseEntry propertyId={propertyId} categories={categories} suppliers={suppliers} act={act}/>:<div className="cost-form cost-form-grid expense-form"><label>Data<input type="date" value={date} onChange={e=>setDate(e.target.value)}/></label><label>Descrição (opcional)<input value={desc} onChange={e=>setDesc(e.target.value)}/></label><label>Categoria<select value={cat} onChange={e=>setCat(e.target.value)}><option value="">Selecionar</option>{categories.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label><label>Fornecedor<select value={sup} onChange={e=>setSup(e.target.value)}><option value="">Sem fornecedor</option>{suppliers.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label><label>Valor total (€)<input inputMode="decimal" value={amount} onChange={e=>setAmount(e.target.value)}/></label><label>IVA (%)<input type="number" value={vat} onChange={e=>setVat(e.target.value)}/></label><label>Estado<select value={status} onChange={e=>setStatus(e.target.value as "paid"|"pending")}><option value="paid">Pago</option><option value="pending">Pendente</option></select></label><label>Nº fatura<input value={invoice} onChange={e=>setInvoice(e.target.value)}/></label><label className="checkbox"><input type="checkbox" checked={rec} onChange={e=>setRec(e.target.checked)}/> Custo recorrente</label><label className="wide">Notas<textarea value={notes} onChange={e=>setNotes(e.target.value)}/></label><button className="primary-button" disabled={!cat||toCents(amount)<=0} onClick={()=>void act(async()=>{const gross=toCents(amount);const rate=vat===""?null:Number(vat);await costsStore.saveExpense({id:edit?.id??crypto.randomUUID(),propertyId,date,description:desc.trim(),categoryId:cat,supplierId:sup||null,grossCents:gross,vatRate:rate,vatCents:vatFromGross(gross,rate),paymentStatus:status,recurring:rec,invoiceNumber:invoice,notes});load();})}>Guardar</button>{edit&&<button className="secondary-button" onClick={()=>load()}>Cancelar</button>}</div>}</article><article className="panel cost-table"><table><thead><tr><th>Data</th><th>Descrição</th><th>Categoria</th><th>Fornecedor</th><th>Estado</th><th>Valor</th><th></th></tr></thead><tbody>{rows.map(x=><tr key={x.id}><td>{x.date}</td><td><strong>{x.description||"—"}</strong>{x.recurring&&<small> · recorrente</small>}</td><td>{categoryName(x.categoryId)}</td><td>{supplierName(x.supplierId)}</td><td>{x.paymentStatus==="paid"?"Pago":"Pendente"}</td><td><strong>{euro(x.grossCents)}</strong></td><td><button onClick={()=>load(x)}>Editar</button><button onClick={()=>void act(()=>costsStore.deleteExpense(propertyId,x.id))}>Remover</button></td></tr>)}</tbody></table></article></>
}

function SalariesTab({propertyId,rows,act}:{propertyId:string;rows:SalaryRecord[];act:(w:()=>Promise<void>)=>Promise<void>}){
 const [edit,setEdit]=useState<SalaryRecord|null>(null);const [month,setMonth]=useState(new Date().toISOString().slice(0,7));const [name,setName]=useState("");const [role,setRole]=useState("");const [gross,setGross]=useState("");const [employer,setEmployer]=useState("");const [meal,setMeal]=useState("");const [other,setOther]=useState("");const [status,setStatus]=useState<"paid"|"pending">("paid");const [notes,setNotes]=useState("");
 ...
 return <><article className="panel"><h2>{edit?"Editar salário":"Novo salário"}</h2><div className="cost-form cost-form-grid"><label>Mês<input type="month" value={month} onChange={e=>setMonth(e.target.value)}/></label><label>Colaborador<input value={name} onChange={e=>setName(e.target.value)}/></label><label>Função<input value={role} onChange={e=>setRole(e.target.value)}/></label><label>Salário bruto (€)<input value={gross} onChange={e=>setGross(e.target.value)}/></label><label>Encargos entidade patronal (€)<input value={employer} onChange={e=>setEmployer(e.target.value)}/></label><label>Subsídio alimentação (€)<input value={meal} onChange={e=>setMeal(e.target.value)}/></label><label>Outros custos (€)<input value={other} onChange={e=>setOther(e.target.value)}/></label><label>Estado<select value={status} onChange={e=>setStatus(e.target.value as "paid"|"pending")}><option value="paid">Pago</option><option value="pending">Pendente</option></select></label><label className="wide">Notas<textarea value={notes} onChange={e=>setNotes(e.target.value)}/></label><div className="salary-total">Custo total <strong>{euro(total)}</strong></div><button className="primary-button" disabled={!name.trim()||total<=0} onClick={()=>void act(async()=>{await costsStore.saveSalary({id:edit?.id??crypto.randomUUID(),propertyId,month,employeeName:name.trim(),role,grossSalaryCents:toCents(gross),employerCostsCents:toCents(employer),mealAllowanceCents:toCents(meal),otherCostsCents:toCents(other),totalCostCents:total,paymentStatus:status,notes});load();})}>Guardar</button>{edit&&<button className="secondary-button" onClick={()=>load()}>Cancelar</button>}</div></article><article className="panel cost-table"><table><thead><tr><th>Mês</th><th>Colaborador</th><th>Função</th><th>Bruto</th><th>Custo total</th><th>Estado</th><th></th></tr></thead><tbody>{rows.map(x=><tr key={x.id}><td>{x.month}</td><td><strong>{x.employeeName}</strong></td><td>{x.role||"—"}</td><td>{euro(x.grossSalaryCents)}</td><td><strong>{euro(x.totalCostCents)}</strong></td><td>{x.paymentStatus==="paid"?"Pago":"Pendente"}</td><td><button onClick={()=>load(x)}>Editar</button><button onClick={()=>void act(()=>costsStore.deleteSalary(propertyId,x.id))}>Remover</button></td></tr>)}</tbody></table></article></>
}