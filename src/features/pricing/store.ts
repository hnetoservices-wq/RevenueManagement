...
function browserConfigs(): PriceManagementConfig[] {
  ...
  if (!raw) return [];
  try { return JSON.parse(raw) as PriceManagementConfig[]; } catch { return []; }
}
...
export class PriceManagementStore {
  ...
  async save(config: PriceManagementConfig): Promise<void> {
    ...
    if (!isTauri()) {
      ...
      const index = configs.findIndex((item) => item.propertyId === config.propertyId);
      if (index === -1) configs.push(config); else configs[index] = config;
      persistBrowser(configs);
      return;
    }
    await (await this.database()).execute(`INSERT INTO price_management_config (property_id,reference_room_type_id,base_prices_json,updated_at)
      VALUES ($1,$2,$3,$4)
      ON CONFLICT(property_id) DO UPDATE SET reference_room_type_id=excluded.reference_room_type_id,base_prices_json=excluded.base_prices_json,updated_at=excluded.updated_at`,
      [config.propertyId, config.referenceRoomTypeId, JSON.stringify(config.basePricesCents), config.updatedAt]);
  }
}
